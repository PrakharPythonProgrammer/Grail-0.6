...
def conv_mimetype(type):
    ...
    if ';' in type:
        ...
        opts = {}
    fields = str.split(str.lower(type), '/')
    if len(fields) != 2:
        raise ValueError("Illegal media type specification.")
    type = str.join(fields, '/')
    return type, opts


def _parse_mimetypeoptions(options):
    opts = {}
    options = str.strip(options)
    while options:
        if '=' in options:
            pos = str.find(options, '=')
            name = str.lower(str.strip(options[:pos]))
            value = str.strip(options[pos + 1:])
            options = ''
            if ';' in value:
                pos = str.find(value, ';')
                options = str.strip(value[pos + 1:])
                value = str.strip(value[:pos])
            if name:
                opts[name] = value
        else:
            options = None
